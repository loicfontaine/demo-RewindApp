...
export async function getAllActivity(): Promise<ZebraActivityRaw[]> {
  startStore();
  const url = `${BASE_URL}/activities?token=${config.public.zebraApiKey}`;
...
export async function getAllRoles(): Promise<RoleRaw[]> {
  startStore();
  const url = `${BASE_URL}/circles?type=all&token=${config.public.zebraApiKey}`;
...
export function postTimesheet({
  ...
  const url = `${BASE_URL}/timesheets?token=${config.public.zebraApiKey}`;
  const data = {
